...
log_and_print() {
  ...
  if [ -f ${log_file} ]; then
      ...
      if (which bc > /dev/null 2>&1); then
        ...
        echo -e "${output}" 2>&1 >> ${log_file}
    fi

    # stdout to console
    if [[ ${silent_debug} -eq 0 ]]; then
        #declare -a message=([0]=$(echo ${1} | cut -d ':' -f 1) [1]=$(echo ${1} | cut -d ':' -f 2- | sed -e 's/^[[:space:]]*//'))
        declare -a message=([0]=$(echo "${1}" | cut -d ':' -f 1) [1]=$(echo "${1}" | cut -d ':' -f 2-))
        case ${debug_flag} in
        0)
            case ${message[0]} in
            "TERM" | "FAIL")
                echo ${echo_flags} -e "${message[1]}"
                ;;
            esac
        ;;
        1 | 2)
...
stdout() {
    ...
    output="TERM: ${1}"
    log_and_print ${echo_flags} "${output}"
}
...
debug() {
    if [[ -n "${1:-}" ]]; then
	if [[ ${debug_flag} -gt 1 ]]; then log_and_print "DEBUG: ${1}"; fi
    else
	while read STDIN; do
	   message=${message:-}$'\n    '${STDIN:-}
	done
	if [[ ${debug_flag} -gt 1 ]]; then log_and_print "DEBUG: ${message:-}"; fi
    fi
}
